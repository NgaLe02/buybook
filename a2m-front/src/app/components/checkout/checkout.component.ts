  ...
  onSubmit() {
    if (this.agreed === true) {
      ...
      for (let i = 0; i < this.bookCodeList.length; i++) {
        subscribeList.push(
          this.dauSachService.getBookAvailable(this.bookCodeList[i])
        );
      }
      forkJoin(subscribeList).subscribe((responseArray) => {
        ...
        for (let i = 0; i < responseArray.length; i++) {
          if (responseArray[i].responseData == "1") {
            sachList.push(
              this.dauSachService.getBookTitleDetail(this.bookCodeList[i])
            );
          } else {
            sachList2.push(
              this.dauSachService.getBookTitleDetail(this.bookCodeList[i])
        ...
        if (sachList.length === 0) {
          ...
          forkJoin(sachList2).subscribe((response) => {
            for (let item of response)
              this.bookListUnavailable.push(item.responseData);
          });
        } else
          forkJoin(sachList)
            .pipe(
              map((response) => {
                ...
                if (_.isEqual(tmp, listTmp)) {
                  ...
                  if (this.borrowedDate)
                    this.phieumuon.borrowDate = this.borrowedDate;
                  else
                    this.phieumuon.borrowDate?.setDate(
                      this.phieumuon.borrowDate.getDate() + 3
                    );

                  this.user0201Service
                    .addPhieuMuon(listTmp, this.phieumuon)
                    .subscribe(
                      (response) => {
                        ...
                        if (response.status === CommonConstant.RESULT_OK) {
                          Swal.fire({
                            ...
                            for (let item of listTmp) {
                              ...
                              this.user0101Service
                                .delete(book)
                                .subscribe((response) => {
                                  if (
                                    response.status === CommonConstant.RESULT_NG
                                  ) {
                                  }
                                });
                            }
                            this.router
                              .navigate(["/user/phieumuon/status/0"])
                              .then(() => {});
                          });
                        } else if (
                          response.status === CommonConstant.RESULT_NG
                  ...
                  listTmp.forEach((val) =>
                    this.bookList.push(Object.assign({}, val))
                  );
                  for (let item of this.bookList) {
                    for (let img of item.images) {
                      if (img.about === 0) {
                        this.imageToShowActiveSrc.push(
                          this.fileService.getFile(img.path)
                        );
                        break;
                      }
                    }
                    ...
                    .pipe(
                      map((response) => {
                        response.forEach((val) =>
                          this.bookListUnavailable.push(
                            Object.assign({}, val.responseData)
                          )
                        );
                        for (let item of this.bookListUnavailable) {
                          for (let item of this.bookListUnavailable) {
                            for (let img of item.images) {
                              if (img.about === 0) {
                                this.imageToShowUnavailableSrc.push(
                                  this.fileService.getFile(img.path)
                                );
                                break;
                              }
