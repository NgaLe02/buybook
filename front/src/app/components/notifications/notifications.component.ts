...
export class NotificationsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
      ...
      this.user0104Service.getNotification().subscribe((notifications) => {
        ...
        for (let i = 0; i < this.notifications.length; i++) {
          if (this.notifications[i].about == 0) {
            for (
              let j = 0;
              j < this.notifications[i]!.book!.images?.length;
              j++
            ) {
              if (this.notifications[i].book?.images[j].about === 0) {
                this.imageToShowSrc.push(
                  this.fileService.getFile(
                    this.notifications[i]!.book!.images[j].path
                  )
                );
                break;
              }
            ...
            this.imageToShowSrc.push("assets/img/notification.png");
          }
        }
      })
    );
    ...
    this.activatedRoute.queryParamMap.subscribe((param) => {
      ...
      this.notifications = [];
      this.user0104Service
        .getNotificationOfUser(this.pageCurrent)
        .subscribe((data) => {});
    });
  }
  ...
  showDetailNotification(notification: Notifications) {
    if (notification.about == 0) {
      ...
      if (notification.phieuMuon?.status == 0) {
        ...
        this.router
          .navigate(["/user/phieumuon/chi-tiet", notification.id], {
