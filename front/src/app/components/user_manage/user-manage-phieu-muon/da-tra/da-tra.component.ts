...
export class DaTraComponent implements OnInit {
  ...
  getData() {
    this.sys0301Service
      .getListPhieuMuonUserLimit(this.id, 2, this.pageCurrent)
      .subscribe({
  ...
  appendQueryParam(page: number) {
    ...
    if (nextPage > 0) {
      this.router.navigate(
        ["/sys/user-detail/" + this.id + "/phieu-muon/status/" + this.status],
  ...
  totalPhieu() {
    this.sys0301Service
      .getTotalPhieuByStatusAndUserUid(this.status, this.id)
      .subscribe({
        next: (resp: DataResponse) => {
          if (resp.status == CommonConstant.RESULT_OK) {
