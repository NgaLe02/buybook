...
export class AuthenticationService {
  ...
  singup() {
    ...
    window.location.href =
      environment.authApiUrl + "/auth/sign-up?redirectUri=" + domain || "";
  }
  ...
  disableUser(userUid: string): Observable<DataResponse> {
    const headers: HttpHeaders = HeadersUtil.getHeadersAuth();
    const url =
      environment.backApiUrl + `/admin/sys0101/disable?userUid=` + userUid;
    return this.http.put<DataResponse>(url, "", { headers: headers });
  }

  activeUser(userUid: string): Observable<DataResponse> {
    const headers: HttpHeaders = HeadersUtil.getHeadersAuth();
    const url =
      environment.backApiUrl + `/admin/sys0101/active?userUid=` + userUid;
    return this.http.put<DataResponse>(url, "", { headers: headers });
  }
