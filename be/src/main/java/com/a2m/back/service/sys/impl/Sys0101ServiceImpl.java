...
public class Sys0101ServiceImpl implements Sys0101Service {
	...
	@Override
	public List<UserResponse> getListUserInfo(String status, String page, String search) {
		HttpHeaders headers = HeadersUtils.setHeaders();
		HttpEntity<List<UserResponse>> entity = new HttpEntity<>(headers);
		ResponseEntity<List<UserResponse>> responseEntity = rest.exchange(
				authApiUrl + "/api/auth/getListUserInfo/" + status + "?page=" + page + "&search=" + search,
	...
	public List<UserResponse> searchByUserId(String userId) {
		HttpHeaders headers = HeadersUtils.setHeaders();
		HttpEntity<List<UserResponse>> entity = new HttpEntity<>(headers);
		ResponseEntity<List<UserResponse>> responseEntity = rest.exchange(authApiUrl + "/api/auth/search/" + userId,
				HttpMethod.GET, entity, new ParameterizedTypeReference<List<UserResponse>>() {
				});
		List<UserResponse> result = responseEntity.getBody();
		return result;
	...
	public UserResponse getUserInfoByUserUid(String userUid) {
		...
		ResponseEntity<UserResponse> responseEntity = rest.exchange(authApiUrl + "/api/auth/userInfo/" + userUid,
	...
	public DataResponse diActiveUser(String USER_UID) {
		...
		if (countPhieuMuonUser > 0) {
			...
			dataResponse.setMessage(
					"Hiện tại user này vẫn còn phiếu cần xác nhận hoặc phiếu chưa trả, vui lòng kiểm tra lại!");
		} else {
			dataResponse.setStatus(CommonConstants.RESULT_OK);
			ResponseEntity<DataResponse> responseEntity = rest.exchange(
					authApiUrl + "/api/user/disable?userUid=" + USER_UID, HttpMethod.PUT, entity, DataResponse.class);
	...
	@Override
	public DataResponse activeUser(String USER_UID) {
		HttpHeaders headers = HeadersUtils.setHeaders();
		HttpEntity<String> entity = new HttpEntity<>(USER_UID, headers);
		DataResponse dataResponse = new DataResponse();
		dataResponse.setStatus(CommonConstants.RESULT_OK);
		ResponseEntity<DataResponse> responseEntity = rest.exchange(authApiUrl + "/api/user/active?userUid=" + USER_UID,
	...
	public List<UserResponse> getAllUserInfo(int size, String page, String search) {
		...
		HttpEntity<List<UserResponse>> entity = new HttpEntity<>(headers);
		ResponseEntity<List<UserResponse>> responseEntity = rest.exchange(
				authApiUrl + "/api/auth/getAllUserInfo?page=" + page + "&size=" + size + "&search=" + search,
				HttpMethod.GET, entity, new ParameterizedTypeReference<List<UserResponse>>() {
				});
		List<UserResponse> result = responseEntity.getBody();
...
//		}
	...
	@Override
	public DataResponse getCountAllUser(String search) {

		HttpHeaders headers = HeadersUtils.setHeaders();
		HttpEntity<Integer> entity = new HttpEntity<>(headers);
		ResponseEntity<DataResponse> responseEntity = rest.exchange(
				authApiUrl + "/api/user/countAllUser?search=" + search, HttpMethod.GET, entity, DataResponse.class);
		DataResponse resp = new DataResponse();
		resp = responseEntity.getBody();
		return resp;
	}
	@Override
	public DataResponse getCountUser(String status, String search) {
		HttpHeaders headers = HeadersUtils.setHeaders();
		HttpEntity<Integer> entity = new HttpEntity<>(headers);
		ResponseEntity<DataResponse> responseEntity = rest.exchange(
				authApiUrl + "/api/user/count?status=" + status + "&search=" + search, HttpMethod.GET, entity,
				DataResponse.class);
		DataResponse resp = new DataResponse();
		resp = responseEntity.getBody();
		return resp;
