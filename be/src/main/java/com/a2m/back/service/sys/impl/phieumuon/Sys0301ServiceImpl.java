...
public class Sys0301ServiceImpl implements Sys0301Service {
	...
	public phieumuonResponse getDetailPhieuMuon(String USER_UID, int STATUS) {
		...
		List<BorrowedBookResponse> bookResponseList = sys0303DAO.getAllBook(phieumuonResponse.getIdPhieuMuon());
		for (BorrowedBookResponse x : bookResponseList) {
	...
	public void xacNhan(int STATUS, int idPhieuMuon, List<String> listIdSelectedBook) throws Exception {
		...
		try {
			...
			// nếu kiểm tra thành công bạn đọc ko có phiếu mượn khác
			// kiểm tra xem đầu sách có thể cho mượn không
			...
			// => những cuốn sách trong trạng thái không là có sẵn vẫn chưa có để người dùng
			// có thể mượn
			// => cho trạng thái của chúng là -1
			if (listBookId.size() > listIdSelectedBook.size()) {
				for (String x : listBookId) {
					if (!listIdSelectedBook.contains(x.substring(0, 8))) {
			...
			notification.setContent("Phiếu mượn có mã là " + idPhieuMuon
					+ " đã được xác nhận thành công. Bạn cần trả sách trước " + DateFor.format(returnDate_Estimate));
...
//            notification.setContent("Phieu muon cua ban da duoc xac nhan thanh cong");
	...
	@Override
	public List<phieumuonResponse> selectByPagination(int STATUS, String page, int statusBorrowDate, String search) {
		List<phieumuonResponse> listRespose = sys0301DAO.selectByPagination(STATUS,
				((Integer.parseInt(page)) - 1) * 15);
		List<phieumuonResponse> result = new ArrayList<>();
		List<String> listUserUid = getAllUserUid(STATUS);
		for (int i = 0; i < listUserUid.size(); i++) {
			UserResponse user = sys0101Service.getUserInfoByUserUid(listUserUid.get(i));
		...
		for (int i = 0; i < result.size(); i++) {
			int idPhieuMuon = result.get(i).getIdPhieuMuon();
			int countBook = getCountBook(idPhieuMuon);
			result.get(i).setCountBook(countBook);
		}
		...
		if (statusBorrowDate == 1) {
			for (phieumuonResponse x : result) {
	...
	public List<phieumuonResponse> selectPhieuMuonByUsername(int status, String username) {
		...
		for (int i = 0; i < listUserUid.size(); i++) {
			...
			if (user.getFullName().toLowerCase().indexOf(username.toLowerCase()) != -1) {
				List<phieumuonResponse> listPhieuMuonByStatusAndUserUid = sys0301DAO.getPhieuMuonInfo(user.getUserUid(),
						status);
