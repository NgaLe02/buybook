...
public class Sys0101Controller {
	...
	public ResponseEntity<DataResponse> getListUserInfo(@PathVariable String status,
			@RequestParam(value = "page", required = false) String page,
			@RequestParam(value = "search", required = false) String search) {
		DataResponse resp = new DataResponse();
		List<UserResponse> listUser;
		try {
			listUser = sys0101Service.getListUserInfo(status, page, search);
	...
	@GetMapping("countListUserInfo/{status}")
	public ResponseEntity<DataResponse> countListUserInfo(@PathVariable String status,
			@RequestParam(value = "page", required = false) String page,
			@RequestParam(value = "search", required = false) String search) {
		DataResponse resp = new DataResponse();
		try {
			resp.setStatus(CommonConstants.RESULT_OK);
			resp.setResponseData(sys0101Service.getCountUser(status, search).getResponseData());
		} catch (Exception e) {
...
//            resp.setStatus(CommonConstants.RESULT_OK);
	...
	public ResponseEntity<DataResponse> activeUser(@RequestParam String userUid) {
		DataResponse resp = new DataResponse();
		try {
			resp = sys0101Service.activeUser(userUid);
//            resp.setStatus(CommonConstants.RESULT_OK);
		} catch (Exception e) {
			resp.setStatus(CommonConstants.RESULT_NG);
			System.out.println(e);
